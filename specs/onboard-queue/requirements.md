...
- **REQ-EVT-12**: WHEN a queue-health report is due and the uplink is connected, the system SHALL publish depth per tier and the counters of REQ-UBI-06 on **`PRIVATE_APP` (256)**, on both the `/2/e/` and `/2/json/` topics, **MQTT-only**: the packet SHALL NOT be transmitted over LoRa and SHALL NOT be queued. On `/2/e/` the payload SHALL be the fixed 90-byte binary record of `design.md` §2.4, because `Data.payload` holds at most 233 bytes (`mesh.pb.h:761`). The JSON serializer SHALL gain an additive `PRIVATE_APP` case that expands that record into the documented JSON object; for any other `PRIVATE_APP` payload it SHALL behave as stock (empty `type`, no `payload` key).
